...
function jomsom_scripts() {
	...
	if ( $options['enable_jcf'] ) {
		wp_register_style( 'jomsom-jcf', trailingslashit( esc_url ( get_template_directory_uri() ) ) . 'css/jcf.css', false, null );
	...
	//For font-awesome
	wp_enqueue_style( 'font-awesome', trailingslashit( esc_url ( get_template_directory_uri() ) ) . 'css/font-awesome.css', false, '4.5.0' );

	wp_enqueue_script( 'jomsom-navigation', trailingslashit( esc_url ( get_template_directory_uri() ) ) . 'js/navigation.min.js', array(), '20120206', true );

	wp_enqueue_script( 'jomsom-skip-link-focus-fix', trailingslashit( esc_url ( get_template_directory_uri() ) ) . 'js/skip-link-focus-fix.min.js', array(), '20130115', true );
	...
	if ( ! $options['disable_scrollup'] ) {
		wp_enqueue_script( 'jomsom-scrollup', trailingslashit( esc_url ( get_template_directory_uri() ) ) . 'js/jomsom-scrollup.min.js', array( 'jquery' ), '20072014', true  );
	}

	//Responsive Menu
    wp_enqueue_script( 'jquery-sidr', trailingslashit( esc_url ( get_template_directory_uri() ) ) . 'js/jquery.sidr.min.js', array('jquery'), '1.2.1.1', false );
    ...
    if ( $options['enable_jcf'] ) {
		 ...
		 * Enqueue JCF
		 */
		wp_enqueue_script( 'jomsom-custom-jcf-script', trailingslashit( esc_url ( get_template_directory_uri() ) ) . 'js/jomsom-custom-jcf-scripts.min.js', array( 'jquery', 'jcf', 'jcf.file', 'jcf.radio', 'jcf.range', 'jcf.number', 'jcf.select', 'jcf.checkbox' ), '1.2.0', true );
	}

	/**
	 * Enqueue custom script for jomsom.
	 */
	wp_enqueue_script( 'jomsom-custom-scripts', trailingslashit( esc_url ( get_template_directory_uri() ) ) . 'js/jomsom-custom-scripts.min.js', array( 'jquery' ), null );

	// Load the html5 shiv on IE 9
	wp_enqueue_script( 'jomsom-html5', trailingslashit( esc_url ( get_template_directory_uri() ) ) . 'js/html5.min.js', array(), '3.7.0' );
 ...
 * Default Options.
 */
require trailingslashit( get_template_directory() ) . 'inc/jomsom-default-options.php';

/**
 * Custom Header.
 */
require trailingslashit( get_template_directory() ) . 'inc/jomsom-custom-header.php';


/**
 * Structure for jomsom
 */
require trailingslashit( get_template_directory() ) . 'inc/jomsom-structure.php';


/**
 * Customizer additions.
 */
require trailingslashit( get_template_directory() ) . 'inc/customizer-includes/jomsom-customizer.php';


/**
 * Custom Menus
 */
require trailingslashit( get_template_directory() ) . 'inc/jomsom-menus.php';


/**
 * Load Featured Content file.
 */
require trailingslashit( get_template_directory() ) . 'inc/jomsom-featured-content.php';


/**
 * Load Slider file.
 */
require trailingslashit( get_template_directory() ) . 'inc/jomsom-featured-slider.php';


/**
 * Load Breadcrumb file.
 */
require trailingslashit( get_template_directory() ) . 'inc/jomsom-breadcrumb.php';


/**
 * Load Widgets and Sidebars
 */
require trailingslashit( get_template_directory() ) . 'inc/jomsom-widgets.php';


/**
 * Load Social Icons
 */
require trailingslashit( get_template_directory() ) . 'inc/jomsom-social-icons.php';


/**
 * Load Metaboxes
 */
require trailingslashit( get_template_directory() ) . 'inc/jomsom-metabox.php';
...
if ( ! function_exists( 'jomsom_continue_reading' ) ) :
	...
	function jomsom_continue_reading() {
		...
		return '&nbsp;<span class="readmore"><a class="more-link" href="' . esc_url( get_permalink() ) . '">' . $more_tag_text . '</a></span><!-- .readmore -->
		';
	}
 ...
 */
function jomsom_custom_css_migrate(){
	...
	if ( function_exists( 'wp_update_custom_css_post' ) ) {
	    ...
	    if ( '' != $options['custom_css'] ) {
	        ...
	        if ( ! is_wp_error( $return ) ) {
	            ...
	            set_theme_mod( 'jomsom_theme_options', $options );
...
add_action( 'after_setup_theme', 'jomsom_custom_css_migrate' );
